...
function makeTerrain(tile, segX, segZ, tex, clip, win) {
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      //
      // 그래서 팔레트는 그대로 두고, 젖은 정도로 살짝 기울이기만 한다.
      // 메마른 데는 먼지빛으로, 넉넉한 데는 풀빛으로.
      ...
      void main(){
        ...
        // 젖은 정도로 땅빛을 살짝 기울인다.
        //   · 메마른 데 → 먼지빛 (유다 광야·네게브·시나이·아라비아·사하라)
        //   · 넉넉한 데 → 풀빛 (분수령 마루·해안 평야·나일 강가·큰 강 유역)
        //
        // 예전에는 이 손질을 **가나안 네모 안에서만** 걸었다. 그러니 네모
        // 가장자리(나바테아·아라비아 사막 어름)에서 밝기가 뚝 끊겨 네모 자국이
        // 드러났다. 손질은 젖은 정도만 보고 거는 것이라 어디에 걸어도 말이
        // 된다 — 네모를 걷어내니 이음매가 사라진다. 팔레트는 여전히 하나다.
        if (hyps < 0.5 && !wet) {
          ...
          float dry = pow(max(1.0 - smoothstep(0.0, 0.62, mo), 0.0), 0.75);
          ...
          float grn = smoothstep(0.36, 0.60, mo);
