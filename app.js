...
function makeTerrain(tile, segX, segZ, tex, clip, win) {
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      //
      // 걸치는 자리는 **아주 넓게, 아주 천천히** 스러지게 잡는다. 예전에는
      // 가나안 네모에 0.5도짜리 테두리로 끊었더니 나바테아·아라비아 사막
      // 어름에서 밝기가 계단처럼 꺾여 네모 자국이 드러났다. 이제 네모를
      // 사방으로 넓히고 테두리를 3도(약 330 km)로 늘여, 손질이 **텅 빈
      // 사막 한복판에서** 눈에 안 띄게 사라지도록 했다.
      float coreAt(float la, float lo){
        float g = 3.0;
        return min(min(smoothstep(0.0, 1.0, (lo - 32.00) / g),
                       smoothstep(0.0, 1.0, (39.00 - lo) / g)),
                   min(smoothstep(0.0, 1.0, (la - 27.00) / g),
                       smoothstep(0.0, 1.0, (35.00 - la) / g)));
      }
