...
function makeTerrain(tile, segX, segZ, tex, clip, win) {
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      //
      // 그래서 팔레트는 그대로 두고, **가나안 안에서만** 젖은 정도로 두 번
      // 살짝 기울인다. 메마른 데는 먼지빛으로, 마루는 풀빛으로. 바깥은
      // 한 획도 건드리지 않는다.
      ...
      // 가나안 정밀 구역 한복판이면 1, 바깥이면 0 (테두리 0.5도에서 스러진다)
      ...
      void main(){
        ...
        // 가나안 안에서만 젖은 정도로 두 번 기울인다.
        //   · 비그늘(유다 광야·요단 골짜기) → 먼지빛
        ...
        if (hyps < 0.5 && !wet) {
          ...
          // 네게브 — 남으로 갈수록 더 황량해진다. 브엘-세바 아래는 이미
          // 사막의 문턱이고, 가데스-바네아쯤이면 온전한 사막이다.
          float neg = (1.0 - smoothstep(30.10, 31.75, la)) * ca;
